...
let eval_eterm f t = 
  let rec eval = function
	...
	| _ -> failwith "[Predicate.eval_term, i2f] wrong types")
    | F2i t -> (match eval t with
	| Float c -> Int (int_of_float c)
	| _ -> failwith "[Predicate.eval_term, f2i] wrong types")
    | Plus (t1, t2) -> 
      (match eval t1, eval t2 with
	| Int c1, Int c2 -> Int (c1 + c2)
	| Float c1, Float c2 -> Float (c1 +. c2)
	| _ -> failwith "[Predicate.eval_term, +] wrong types")
    | Minus (t1, t2) -> 
      (match eval t1, eval t2 with
	| Int c1, Int c2 -> Int (c1 - c2)
	| Float c1, Float c2 -> Float (c1 -. c2)
	| _ -> failwith "[Predicate.eval_term, binary -] wrong types")
    | Mult (t1, t2) -> 
      (match eval t1, eval t2 with
	| Int c1, Int c2 -> Int (c1 * c2)
	| Float c1, Float c2 -> Float (c1 *. c2)
	...
	| _ -> failwith "[Predicate.eval_term, mod] wrong types")
    | UMinus t -> 
      (match eval t with
	| Int c -> Int (- c)
	| Float c -> Float (-. c)
	| _ -> failwith "[Predicate.eval_term, unary -] wrong type")
  in
  eval t
